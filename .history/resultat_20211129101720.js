    ...
    if(localStorage.getItem('Etudiants')){
        ...
        for (let etudiant in arr) {
            ...
            <tr class="user" id="${id}" onclick="afficheInfo(${etudiant})">
                ...
                <td><p class="appreciation" onload="Appreciation(${etudiant})">${arr[etudiant].appreciation}</p></td>
            </tr>
            `;
            id++;
...
function Appreciation(etudiant){
    ...
    let result = document.querySelector('.appreciation');
        if(ParseInt(arr[etudiant].appreciation) <= 40){
            result.style.background="red";
        }else if(ParseInt(arr[etudiant].appreciation) <= 64){
