    ...
    if(localStorage.getItem('Etudiants')){
        ...
        for (let etudiant in arr) {
            ...
            <tr class="user" id="${id}" onclick="afficheInfo(${etudiant})">
                <td><img class="photoEtudiant" style="width:50px;border-radius: 50%;object-fit: cover;" src="url(${arr[etudiant].photo})" alt=""></td>
                <td>&nbsp; 00${id}</td>
...
function ajouterNoteEtudiant(rid) {
    let arr = JSON.parse(localStorage.getItem('Etudiants'));
    arr[rid].pointRequi = document.getElementById('totalRequis').value;
    arr[rid].pointObtenu = document.getElementById('totalObtenus').value;
