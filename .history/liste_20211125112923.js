    ...
    if(localStorage.getItem('Etudiants')){
        ...
        for (let etudiant in arr) {
            ...
            <tr class="user ${arr[etudiant].specialite}" id="${id}" onclick="afficheInfo(${etudiant})">
                ...
                <td>&nbsp; <button onclick="event.stopPropagation();supStudent(this)"><span class="fa fa-times"></span></button> </td>
            </tr>
            `;
            id++;
        }
...
function supStudent(td){
        if (confirm('Supprimer')) {
            row = td.parentElement.parentElement;
            document.querySelector('.user').deleteRow(row.rowIndex)
        }
}